...
"""
Usage:
    ecdklint [options] [FILE_OR_DIR ...]

Options:
    -c, --config-file FILE  Path to a custom configuration file.
    -d, --config-data DATA  Custom configuration as YAML source.
...
from ecdklint import APP_DESCRIPTION, APP_NAME, APP_VERSION, linter
from ecdklint.config import ECdkLintConfig, ECdkLintConfigError
from ecdklint.linter import PROBLEM_LEVELS
        ...
        if os.path.isdir(item):
            for root, _dirnames, filenames in os.walk(item):
                for f in filenames:
                    ...
                    if conf.is_yaml_file(filepath) and not conf.is_file_ignored(
                        filepath
                    ):
                        yield filepath
        else:
...
def find_project_config_filepath(path="."):
    for filename in (".ecdklint", ".ecdklint.yaml", ".ecdklint.yml"):
...
def run(argv=None):
    ...
    if "ECDKLINT_CONFIG_FILE" in os.environ:
        user_global_config = os.path.expanduser(os.environ["ECDKLINT_CONFIG_FILE"])
    # User-global config is supposed to be in ~/.config/ecdklint/config
    elif "XDG_CONFIG_HOME" in os.environ:
        user_global_config = os.path.join(
            os.environ["XDG_CONFIG_HOME"], "ecdklint", "config"
        )
    else:
        user_global_config = os.path.expanduser("~/.config/ecdklint/config")
    ...
    try:
        if args.config_data is not None:
            ...
            conf = ECdkLintConfig(content=args.config_data)
        elif args.config_file is not None:
            conf = ECdkLintConfig(file=args.config_file)
        elif project_config_filepath:
            conf = ECdkLintConfig(file=project_config_filepath)
        elif os.path.isfile(user_global_config):
            conf = ECdkLintConfig(file=user_global_config)
        else:
            conf = ECdkLintConfig("extends: default")
    except ECdkLintConfigError as e:
