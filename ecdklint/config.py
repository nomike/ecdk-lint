...
import ecdklint.rules
from ecdklint import decoder


class ECdkLintConfigError(Exception):
    pass


class ECdkLintConfig:
    ...
    def extend(self, base_config):
        assert isinstance(base_config, ECdkLintConfig)
    ...
    def parse(self, raw_content):
        try:
            ...
            raise ECdkLintConfigError(f"invalid config: {e}") from e

        if not isinstance(conf, dict):
            raise ECdkLintConfigError("invalid config: not a mapping")

        self.rules = conf.get("rules", {})
        if not isinstance(self.rules, dict):
            raise ECdkLintConfigError(
...
def validate_rule_conf(rule, conf):
    ...
    if isinstance(conf, dict):
        ...
        if hasattr(rule, "VALIDATE"):
            ...
            if res:
                raise ECdkLintConfigError(f"invalid config: {rule.ID}: {res}")
    else:
        raise ECdkLintConfigError(
...
def get_extended_config_file(name):
    # Is it a standard conf shipped with ecdklint...
    if "/" not in name:
