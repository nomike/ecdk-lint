...
def detect_encoding(stream_data):
    ...
    Before this function was added, ecdklint would sometimes decode text files
    using a non-standard character encoding. It’s possible that there are users
    out there who still want to use ecdklint with non-standard character
    encodings, so this function includes an override switch for those users. If
    the ECDKLINT_FILE_ENCODING environment variable is set to "example_codec",
    ...
    """
    if "ECDKLINT_FILE_ENCODING" in os.environ:
        warnings.warn(
            "ECDKLINT_FILE_ENCODING is meant for temporary "
            "workarounds. It may be removed in a future version of "
            "ecdklint.",
            stacklevel=2,
        )
        return os.environ["ECDKLINT_FILE_ENCODING"]
